...
$addBtn             = '<button type="button" data-whatever="'.$html->lang['AddRemoteJP'].'" data-target="#dialogModal" data-fn="addRemote" class="dialogModal btn btn-sm btn-success pull-right">'.$html->lang['AddRemoteJP'].'</button>';
$updateAdminBtn     = '';
$cssImport          = [];
...
try {
	...
	do{
		...
		$db->row['RemoteState'] = '<span class="text-'.$statusClassAry[$status].'">'.$html->lang['Status'.$statusAry[$status]].($html->WebLogin->ag_id == '1' && $status==0 ? '('.$db->row['RemoteState'].')' : '').'</span>';
